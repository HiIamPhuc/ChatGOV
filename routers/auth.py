...
COOKIE_SAMESITE = os.getenv("COOKIE_SAMESITE", "lax")  # "lax" (local) / "none" (prod)

# Frontend origin & redirect links (có thể override từng link riêng)
FRONTEND_ORIGIN = os.getenv("FRONTEND_ORIGIN", "http://localhost:5173").rstrip("/")
AUTH_CONFIRM_REDIRECT = os.getenv("AUTH_CONFIRM_REDIRECT") or f"{FRONTEND_ORIGIN}/signin"
AUTH_RESET_REDIRECT = os.getenv("AUTH_RESET_REDIRECT") or f"{FRONTEND_ORIGIN}/reset"
...
def _no_store(resp: Response) -> None:
    # Chặn cache cho các response nhạy cảm (auth/session)
...
def register(body: RegisterBody):
    try:
        # ENV: AUTH_CONFIRM_REDIRECT hoặc FRONTEND_ORIGIN/signin
        confirm_redirect = AUTH_CONFIRM_REDIRECT or f"{FRONTEND_ORIGIN}/signin"
        res = client.auth.sign_up(
            {
                ...
                "options": {
                    "data": (
                        {"fullName": body.fullName, **(body.metadata or {})}
                        if body.fullName
                        else (body.metadata or {})
                    ) or {},
...
def forgot_password(body: ForgotBody):
    try:
        # ENV: AUTH_RESET_REDIRECT hoặc FRONTEND_ORIGIN/reset
        redirect_to = AUTH_RESET_REDIRECT or f"{FRONTEND_ORIGIN}/reset"
...
def me(request: Request, authorization: str | None = Header(None), response: Response = None):
    token = request.cookies.get(ACCESS_COOKIE)
    if not token and authorization and authorization.lower().startswith("bearer "):
        token = authorization.split(" ", 1)[1]
    if not token:
        if response is not None: _no_store(response)
