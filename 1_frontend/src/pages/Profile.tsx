...
const Wrap = styled.div`
  ...
  h1, h2 { margin: 0; font-size: 1.25rem; color: ${({ theme }) =>
    theme.colors.accent2}; }
  .subtitle { margin: 4px 0 0; color: ${({ theme }) =>
    theme.colors.secondary}; font-size: .95rem; }
  ...
  .btn {
    ...
    color: ${({ theme }) =>
      theme.colors.primary}; font-weight: 700; cursor: pointer;
    transition: transform .06s ease, filter .15s ease, box-shadow .15s ease;
  }
  .btn:hover { background: #fff5ef; }
  .btn.accent { background-image: linear-gradient(90deg, ${({ theme }) =>
    theme.colors.accent} 0%, ${({ theme }) =>
  theme.colors
    .accent2} 100%); color:#fff; box-shadow:0 6px 16px rgba(206,122,88,.35); }
  .btn.accent:hover { filter: brightness(.96); } .btn.accent:active { transform: translateY(1px); filter: brightness(.92); }
  .btn.ghost { background:#fff; border:1px solid ${({ theme }) =>
    theme.colors
      .border}; } .btn.ghost:hover { background:#fff5ef; border-color:#f0d2c5; }

  .grid { display:grid; grid-template-columns: repeat(auto-fit,minmax(220px,1fr)); gap:12px; }
  .pwGrid { grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); }

  label { display:flex; flex-direction:column; gap:6px; font-weight:600; color:${({
    theme,
  ...
  input {
    height: 38px; padding: 0 10px; border-radius: 10px; border:1px solid ${({
      theme,
    }) => theme.colors.border};
  ...
  input:focus { border-color:${({ theme }) =>
    theme.colors.accent}; box-shadow:0 0 0 3px rgba(206,122,88,.2); }
  input:disabled { background:${({ theme }) =>
    theme.colors.surface2}; color:${({ theme }) =>
  theme.colors.secondary}; cursor:not-allowed; }
  ...
  .dateField .calendarBtn { position:absolute; right:6px; top:50%; transform:translateY(-50%); width:30px; height:30px; border:none; border-radius:8px; background:transparent; color:${({
    theme,
  }) =>
    theme.colors.secondary}; display:grid; place-items:center; cursor:pointer; }
  .dateField .calendarBtn:hover { color:${({ theme }) =>
    theme.colors.accent}); background:#fff5ef; }
