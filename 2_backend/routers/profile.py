...
def _validate_profile_patch(patch: dict):
    ...
    if "age" in patch and patch["age"] is not None:
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Tuổi phải là số nguyên")
        ...
        if isinstance(patch.get("dob"), date) and _calc_age_from_dob(patch["dob"]) != age_int:
            raise HTTPException(
                status_code=400, detail="Tuổi không khớp với ngày sinh")
...
def change_password(body: ChangePasswordBody, request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=401, detail="Mật khẩu hiện tại không hợp lệ")
    try:
        admin_client.auth.admin.update_user_by_id(
            uid, {"password": body.next_password})
        return {"ok": True}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)
                            or "Đổi mật khẩu thất bại")
