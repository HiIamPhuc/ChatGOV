...
from routers.profile import _get_current_user
        ...
        for r in rows:
            ...
            out.append(
                SessionOut(
                    ...
                    last_message_preview=(messages and str(messages[-1].content)[:180])
                    or None,
...
def get_history(request: Request, session_id: str = Query(...)):
    uid, _ = _get_current_user(request)
...
def rename_session(request: Request, session_id: str = Path(...), payload: RenameRequest = Body(...)):
    try:
        uid, _ = _get_current_user(request)
        sess = get_session(session_id)
        if not sess or sess.user_id != uid:
        ...
        raise HTTPException(
            status_code=500, detail=str(e) or "Failed to rename session"
        )


@router.post("/api/chat/sessions/{session_id}/autoname", response_model=dict)
def autoname_session(request: Request, session_id: str = Path(...)):
    """
    ...
    try:
        ...
        if not user_messages:
            raise HTTPException(
                status_code=400,
                detail="No user messages found to generate a title from.",
            )
...
def delete_session(request: Request, session_id: str = Path(...)):
    uid, _ = _get_current_user(request)
    user = get_user(uid)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    try:
        ...
        raise HTTPException(
            status_code=500, detail=str(e) or "Failed to delete session"
        )
